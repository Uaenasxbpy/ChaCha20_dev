...
int crux_entry_chacha20_roundtrip(void)
{
    uint8_t key[32];
    uint8_t nonce[12];
    ...
    for (int i = 0; i < 32; i++) {
        key[i] = crucible_uint8_t("key");
    }

    for (int i = 0; i < 12; i++) {
        nonce[i] = crucible_uint8_t("nonce");
    }

    for (int i = 0; i < 64; i++) {
        plaintext[i] = crucible_uint8_t("plaintext");
    }

    uint32_t counter = crucible_uint32_t("counter");

    chacha20_encrypt(key, counter, nonce, plaintext, 64, ciphertext);
    chacha20_decrypt(key, counter, nonce, ciphertext, 64, decrypted);
...
int crux_entry_chacha20_block_rfc8439(void)
{
    uint8_t key[32] = {
        0x00, 0x01, 0x02, 0x03,
        0x04, 0x05, 0x06, 0x07,
    ...
    chacha20_block(output, key, 1, nonce);
...
int crux_entry_chacha20_encrypt_rfc8439(void)
{
    uint8_t key[32] = {
        0x00, 0x01, 0x02, 0x03,
        0x04, 0x05, 0x06, 0x07,
    ...
    chacha20_encrypt(key, 1, nonce, plaintext, 114, ciphertext);

    for (int i = 0; i < 114; i++) {
        check(ciphertext[i] == expected[i]);
    }
